...
public class GeozetServlet extends ServletBase {
    ...
    protected void service(HttpServletRequest request,
        ...
        // Thijs: houdt rekening met de layout, als die aanwezig is in de URI
        ...
        if ((xcoord > 0) && (ycoord > 0) && (straal > 0)) {
            ...
            final RequestDispatcher rd = this.getServletContext()
                    .getRequestDispatcher("/" + this.layout + this._BEKENDMAKINGEN);
            rd.forward(request, response);
            return;
        }

        // controle voor OLS param's, als het zoekveld is gevuld adres opzoeken
         String zoekveld = request.getParameter(REQ_PARAM_ADRES.code);
        if ((zoekveld != null) /* && (zoekveld != "") */) {
            LOGGER.debug("Forwarding naar de OLS client.");
            final RequestDispatcher rd = this.getServletContext()
                    .getRequestDispatcher("/"+ this.layout + this._LOCATIE);
            rd.forward(request, response);
            return;
        }
        ...
        // String pagina = "";
        // Houdt rekening met de layout hier?
        LOGGER.debug("Forwarding naar de start pagina.");
        final RequestDispatcher rd = this.getServletContext()
                .getRequestDispatcher("/" + this.layout + "index.jsp");
        rd.forward(request, response);
    }
