...
var geozetENHANCED = {
    ...
    resetHTML: function() {
        ...
        // Thijs Brentjens, mei 2014: alleen als er vlakbekendmakingen zijn
        if (Geozet.config.vlakgerichteBekendmakingenService!=undefined) {
        ...
        if ((Geozet.config.containers.miniMap === false) && (Geozet.config.containers.filter === false)) { 
            // Thijs: make sure this only happens when the map is used for Overheid.nl
            // The radius-map does not need to overrule the staticMapSize
            // TODO: if the map is used in different sizes, this might mean that some re-coding is needed for the sizes. For now, just add another check and assume the staticMapSize is set correctly in the settings if the radius map is needed.
            if (!Geozet.config.showRadius) Geozet.config.staticMapSize = [948, 550];
        ...
        if ( $('#geozetContent.start').length !== 0 ){
            startC = "";
            if ($('#geozetCore')) {
                startC = $('#geozetCore').html();
            }
            $('#geozetCore, #geozetAsideCore').remove();
            if (!Geozet.config.showRadius && (document.location.hash === "" || document.location.hash === "#")) {
				...
				$('a.geozetClose').focus();
            } else if (Geozet.config.showRadius && OpenLayers.Util.getParameters().straal != undefined) {
                // TODO: limit the list?
                $('#geozetMap').append('<div id="geozetStart"><div class="geozetStart"><a href="#" class="geozetClose" title="'+OpenLayers.i18n("geozetStartCloseTitle")+'">'+OpenLayers.i18n("geozetStartCloseText")+'</a>'+startC+'</div>');
                $('#adres').addClass('autopopulate').attr('title',OpenLayers.i18n("searchFieldTitle"));
                $('a.geozetClose').focus();
