...
// determine te root of the application, to be used to construct the paths for images and the proxy
Geozet.config.appname = location.pathname.split("/")[1];

// root path from where the icons for in the map can be retrieved
Geozet.config.imgPath = "/" + Geozet.config.appname + "/static/img/";

OpenLayers.ImgPath = Geozet.config.imgPath; 
OpenLayers.IMAGE_RELOAD_ATTEMPTS = 2;
OpenLayers.ProxyHost = "/" + Geozet.config.appname + "/proxy.jsp?";

// urls
// Geozet.config.bekendmakingenWFS = '/wfs?';
...
Geozet.config.overview = {
    ...
    layers: [new OpenLayers.Layer.Image(null, "/"+Geozet.config.appname+"/static/img/kaart-nl.gif", new OpenLayers.Bounds(7216,301377,284047,622933), new OpenLayers.Size(153, 179),
...
Geozet.config.mapAutoWidth = true;
// minimum size for the map
Geozet.config.staticMapSize = [300, 300];

// ordening van features in de kaart op basis van instelbare rijhoogte in pixels
Geozet.config.sortHeight = 50;

// urlitems are the items that can be in the url in the order of importance (provincie more important than gemeente):
Geozet.config.urlitems = ["provincie","gemeente","plaats","wijk","buurt","postcode","straat","huisnummer"];
...
// Thijs Brentjens: for this map, show the radius if possible
Geozet.config.showRadius = true;
Geozet.config.radiusStyle = {
                                fillOpacity: 0.4
                                , fillColor:"#ffbb99"
                                , strokeColor: "#ffbb99"
                                , strokeWidth: 2                                
                            };
Geozet.config.pointStyle = {
                                fillOpacity: 0.8
                                , fillColor:"#ff0000"
                                , pointRadius: 5
                                // , externalgraphic?
