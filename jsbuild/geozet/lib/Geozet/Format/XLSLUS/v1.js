...
Geozet.Format.XLSLUS.v1 = OpenLayers.Class(OpenLayers.Format.GML, {
	...
	"gml": {
    	    "Point": function(node, feature) {
                ...
                // only if no geometry is there already, use a point
                if (feature.geometry == undefined || feature.geometry == null) feature.geometry = geometry;
    	    },
            "MultiPolygon": function(node, feature) {
                var parser = this.parseGeometry["multipolygon"]; 
                var geometry = parser.apply(this, [node]);
                if (this.internalProjection && this.externalProjection) {
                    geometry.transform(this.externalProjection,
                                       this.internalProjection);
            ...
            /*,"Polygon": function(node, feature) {
                var parser = this.parseGeometry["polygon"]; 
                var geometry = parser.apply(this, [node]);
                if (this.internalProjection && this.externalProjection) {
                    geometry.transform(this.externalProjection,
                                       this.internalProjection);
            ...
            "GeocodeRequest": function(address) {
                var root = this.createElementNSPlus(
                    ...
                    {attributes: {
                        ...
                        // Thijs Brentjens: for PDOK there is a custom parameter to get polygons as well: includePolygons
