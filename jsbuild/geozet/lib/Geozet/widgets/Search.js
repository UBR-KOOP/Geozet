...
Geozet.Search = OpenLayers.Class({
    ...
    initialize: function(config, map) {
        ...
        $('#geozetEnhancedSearch, #geozetCoreEntree').delegate('li/a', 'click', function () {
            ...
            if(x && y){
                ...
                search.mapZoom = z;
                $('#geozetStart').remove();
                search.setLastHash(hash);
                document.location.hash = hash;
                var center = new OpenLayers.LonLat(x, y);
                map.setCenter(center, z);
                // Thijs Brentjens: if we are in a map only where the radius needs to be set,
                // then create that one here
                if (Geozet.config.showRadius) {
                    // TODO: create a spearate plugin for this, using own events?
                    // For now it is sufficient.
                    // Tip from: http://stackoverflow.com/questions/3442394/jquery-using-text-to-retrieve-only-text-not-nested-in-child-tags
                    var address = $("*", this).contents().filter(function(){ 
                                              return this.nodeType == 3; 
                                            })[0].nodeValue;                     
                    Geozet.Viewer.createRadius(center, address);
                }
            }
            else {
    ...
    /**
    * Function called at the end of every mapmove (zoom/pan/ect)
    * Search-control is listening to mapmove events
    *
    * it either tries to set a geocoder hash-OR an xyz-hash
    */
    mapMoveEnd: function() {
        if(this.map.getCenter() && this.mapCenterX && this.mapCenterY){
            var hor = this.mapCenterX - this.map.getCenter().lon;
            var vert = this.mapCenterY - this.map.getCenter().lat;
            var TRESHOLD = 10;
            var hash = '';
            if (vert > TRESHOLD){
                hash+='/zuid/'+Math.round(vert)+'m';
            }
            else if (vert < -TRESHOLD){
                hash+='/noord/'+Math.round(Math.abs(vert))+'m';
            }
            if (hor > TRESHOLD){
                hash+='/west/'+Math.round(hor)+'m';
            }
            else if (hor < -TRESHOLD){
                hash+='/oost/'+Math.round(Math.abs(hor))+'m';
            }
            if (this.map.getZoom() != this.mapZoom){
                hash+='/zoomniveau/'+this.map.getZoom();
            }
            document.location.hash=this.lastHash+hash;
        }
        else if (this.map.getZoom()>0 && !(this.lastHash && this.lastHash.length()>0)) {
            // meaning we are NOT at init zoom, and there is NO geocoder hash: place an xyzoomniveau hash
            document.location.hash='/x/'+Math.round(this.map.getCenter().lon)+'/y/'+Math.round(this.map.getCenter().lat)+'/zoomniveau/'+this.map.getZoom();
        }
    ...
    showError: function(msg) {
        if ($('#'+this.searchFormId).length==0){
            // default to geozetEnhancedSearch
            this.setSearchForm('geozetEnhancedSearch');
        }
    ...
    sendRequestFromHash: function(hashAddressString, handleResponse) { 
        ...
        */
        ...
        /* PDOK also supports a custom parameter :includePolygons, that results in not only points, but also polygons for Geocoding
            
        <GeocodeRequest xmlns="http://www.opengis.net/xls" xsi:schemaLocation="http://www.opengis.net/xls http://schemas.opengis.net/ols/1.1.0/LocationUtilityService.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" includePolygons="true">
            <Address countryCode="NL">
                <StreetAddress>
                    <Building></Building>
                    <Street></Street>
        ...
        */
        // to make sure we can handle also #gemeente/haarlem
        if ( hashAddressString.indexOf('#/')<0 && hashAddressString.indexOf('/')!=0){
            hashAddressString=hashAddressString.replace('#','#/');
        }
        this.setLastHash(hashAddressString);
     ...
     *
     * Also sets the geocoder-hash if needed
     * 
     * Parameters:
    ...
    handleResponse: function(req, returnCoords){
        ...
        var hits=(!xlslus[0])?0:xlslus[0].numberOfGeocodedAddresses;
        if (hits==0){
            // zero responses
            ...
            for (i=0;i<hits;i++){
                ...
                if (address.postalCode){
                    // TODO: Thijs, how to deal with the dash?
                ...
                if(hits>1){
                    ...
                    if (geom) {
                        if (geom.CLASS_NAME =="OpenLayers.Geometry.MultiPolygon" || geom.CLASS_NAME =="OpenLayers.Geometry.Polygon") {
                        var bounds = geom.getBounds();
                        z = this.map.getZoomForExtent(bounds);
                        x = bounds.getCenterLonLat().lon;
                        y = bounds.getCenterLonLat().lat;
                        }
                    }
                    ...
                    // handle hashes which hold zuid, noord, west or oost (value in meters, like: 1500m)
                    // and zoomniveau
                    // these are NOT set as part of lastHash, because this hash is the startingpoint
                    // example: geozetviewer#/provincie/NOORD-HOLLAND/gemeente/HAARLEM/zuid/1000m
                    var zuid = parseInt(this.findUrlHashValue(this.lastHash.split('/'), 'zuid'));
                    var noord = parseInt(this.findUrlHashValue(this.lastHash.split('/'), 'noord'));
                    var west = parseInt(this.findUrlHashValue(this.lastHash.split('/'), 'west'));
                    var oost = parseInt(this.findUrlHashValue(this.lastHash.split('/'), 'oost'));
                    var zoom = parseInt(this.findUrlHashValue(this.lastHash.split('/'), 'zoomniveau'));
                    if(!isNaN(zuid)){
                        y-=zuid;
                    }
                    if(!isNaN(noord)){
    ...
    destroy: function() {
        ...
        this.map.events.unregister("moveend", this.mapMoveEnd)
        this.map=null;
        this.config=null;
