...
Geozet.Viewer = function() {
    ...
    return {
        ...
        create: function(config) {
            ...
            this.createMap();
            this.createSearch();
            // Thijs Brentjens: add the possibility to switch off bekendmakingen layers, because of the single map function (added later, may 2014)
            // This could be done more generic, but that would require more refactoring
            if(this.config.vlakgerichteBekendmakingenService != undefined) this.createVlakgericht();

            // Thijs Brentjens: add the possibility to switch off bekendmakingen layers, because of the single map function (added later, may 2014)
            // This could be done more generic, but that would require more refactoring            
            if(this.config.bekendmakingenWFS != undefined) {
                ...
                // Thijs Brentjens: only do this if a map with a popup layer has been defined
            ...
            var searchCb = function(req) {
                ...
                // Thijs Brentjens: show the radius of an address, if possible. This needs explicit configuration in the settings                
                if (this.config.showRadius) {                 
                    if (returnObj.center) {
                        this.createRadius(returnObj.center, returnObj.address); 
                        $('#geozetStart').remove();
                        document.location.hash = returnObj.hash;
                    }
                }
            };
            var cb = OpenLayers.Function.bind(searchCb, this);
            if (startHash != "" && startHash != "#"){
                // either a geocoder-hash (/provincie/...) or an xyz-hash
                // check for /x/.../y/...
                var x = parseInt(search.findUrlHashValue(startHash.split('/'), 'x'));
                var y = parseInt(search.findUrlHashValue(startHash.split('/'), 'y'));
                if (isNaN(x) && isNaN(y)){
                    // no x and y, so probably a geocoder hash, try to solve it
                    search.setSearchForm("geozetCoreEntree");
                    search.sendRequestFromHash(startHash, cb);
                }
                else{
                    // found an x AND an y, check for zoomniveau
                    var z = parseInt(search.findUrlHashValue(startHash.split('/'), 'zoomniveau'));
                    if (isNaN(z)){
                        z= Geozet.config.zoomScale.standaard;
                    }
                    this.createMapPanel(new OpenLayers.LonLat(x, y), z);
                }
            } else {
                this.createMapPanel();
            }
        ...
        createMap: function() {
            ...
            // Thijs: the attachControl function is moved

        },/**
         * Method: createRadius
         * Create a Vectorlayer on the {<OpenLayers.Map>} object for the main map and show the radius. Zoom accordingly.
         * The radius is a URL parameter, called 'straal', containing the radius in meters. Not a hash part, since the hash is for the location of the map only.         
         */
        createRadius: function(center, addressString) {
            var urlParams = OpenLayers.Util.getParameters(document.location.href);
            // TODO: default to 500 meter, or use config object?
            var radius = 500;
            var preferredZoom = 12; // default no preferredzoom        
            if (urlParams) {
                radius = parseInt(urlParams.straal);
                if (urlParams.maxzoomniveau) preferredZoom = urlParams.maxzoomniveau;
            } 
            if (addressString== undefined) addressString = "";
            // TODO: show the adresssString?  or postcode en huisnummer?          
            $('#kaarttoelichting').text("U ontvangt berichten binnen een straal van "+ radius+ " meter rondom het adres "+addressString+". Het gebied is op de kaart hieronder aangegeven.");
            // create a temp map layer (or replace it if already exists)
            var radiusLayer = new OpenLayers.Layer.Vector("Straal rondom adres");
            // Use a config object for the styling, otherwise use this default styling object
            var radiusStyle = {                                
                                fillOpacity: 0.3,
                                fillColor:"#ffff00"
                            }
            if (Geozet.config.radiusStyle!=undefined) radiusStyle = this.config.radiusStyle;
            var pointStyle = {                                
                                fillOpacity: 0.6,
                                fillColor:"#ff0000",
                                pointRadius: 5
                            }
            if (Geozet.config.pointStyle!=undefined) pointStyle = this.config.pointStyle;
            ...
            (
                centerPoint,
                radius,
                80, // 80 parts to approximate a circle
                0
            );

            var centerFeature = new OpenLayers.Feature.Vector(
                    centerPoint,
                    {"address": addressString},// only the address if available
                    pointStyle
                );
            var radiusFeature = new OpenLayers.Feature.Vector(
             ...
             radiusLayer.addFeatures([radiusFeature, centerFeature]);
             map.addLayer(radiusLayer);
             // if the zoomniveau is set in the URL, use that as minimum, otherwise, zoom to the best fit.
             // or get a level different?
             var zoomLevel = Math.min(map.getZoomForExtent(radiusLayer.getDataExtent()), preferredZoom);
             
             map.setCenter(new OpenLayers.LonLat(center.lon, center.lat), zoomLevel);
             // map.zoomToExtent(radiusLayer.getDataExtent());
        },
        /**
         * Method: destroy
         * Clean up.
